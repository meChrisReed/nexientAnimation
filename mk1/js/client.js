...
$(document).ready(function () {
	...
	function createParticle (maxPArticles, upperDistance, lowwerDistance) {
		...
		// 	// context.beginPath();
		// 	// context.arc( 0, 0, 0.5, 0, 6.28318, true );
		// 	// context.fill();
		// 	var	pos = 0.5;
    //   var radius = 0.5;
    ...
    //   context.fill();
		// };
		// var particle = new THREE.Sprite(new THREE.SpriteCanvasMaterial( { color: color, program: drawTri} ));
	    ...
	    particles.vertices.forEach(function (particle) {
				...
				if (
					...
					(function () {
						...
						var lifeRemainaing = 1 - (particle.life.getElapsedTime() / particle.lifeDuration);
					...
					(function setNewNoiseDestination () {
						...
						particle.noiseDuration = Math.round(Math.random() * 2000) + 200; // in miliseconds
					}());
					if (!particle.life) {
						particle.life =  new THREE.Clock();
						particle.life.start();

						particle.lifeDuration = (Math.random() * 2) + 0.75; // in mils
					}
				}
			});
		}());
